...
void setup() {
    // StartBTN
  //  pinMode(20, INPUT_PULLUP); //Start BTN
    // FX buttons
  pinMode(9, INPUT_PULLUP); // FX-L
  pinMode(10, INPUT_PULLUP); // FX-R
    //BT buttons
  pinMode(7, INPUT_PULLUP); //BT-A
  pinMode(6, INPUT_PULLUP); // BT-B
  pinMode(14, INPUT_PULLUP); // BT-C
  pinMode(15, INPUT_PULLUP); // BT-D
    //Start BTN Light
 // pinMode(21, OUTPUT); //START BTN Light
    //FX LIGHTS
  pinMode(8, OUTPUT); // FX-L LIGHT
  pinMode(16, OUTPUT); // FX-R LIGHT
    //BT Lights
  pinMode(4, OUTPUT); //BT-A Light
  pinMode(5, OUTPUT); //BT-B Light
  pinMode(A0, OUTPUT); // BT-C LIGHT
  pinMode(A1, OUTPUT); // BT-D LIGHT
...
void loop() {
  ...
  /*if (digitalRead(20) == LOW) {
    Keyboard.press(0xB0);
    if(SdvxComp == true || LightsReverseOn == true) digitalWrite(21, LOW);
      else  digitalWrite(21, HIGH);
  } else if (digitalRead(20) == HIGH) {
      Keyboard.release(0xB0);
    if(SdvxComp == true || LightsReverseOn == true) digitalWrite(21, HIGH);
      else   digitalWrite(21, LOW);
  }*/
    //press both FX-L and FX-R and Start BTn to press ESC
  if(digitalRead(20) == LOW, digitalRead(7) == LOW || digitalRead(6) == LOW || digitalRead(14) == LOW ||digitalRead(15) == LOW) {
    Keyboard.press(0xB1);
  } else if(digitalRead(20) == HIGH, digitalRead(7) == HIGH || digitalRead(6) == HIGH || digitalRead(14) == HIGH || digitalRead(15) == HIGH) {
      Keyboard.release(0xB1);
    }
    //Press FX-L and FX-R, BT-D and START BTN to toggle Sdvx compability
  if(digitalRead(20) == LOW && digitalRead(15) == LOW && digitalRead(9) == LOW && digitalRead(10) == LOW && On == false){
    SdvxComp = !SdvxComp;
    On = true;
  }else if(digitalRead(20) == HIGH && digitalRead(15) == HIGH && digitalRead(9) == HIGH && digitalRead(10) == HIGH && On == true) On = false;
  
   //Press FX-L and FX-R, BT-C And Start BTN to toggle light Reverse
  if(digitalRead(20) == LOW && digitalRead(14) == LOW && digitalRead(9) == LOW && digitalRead(10) == LOW && On == false) {
     LightsReverseOn = !LightsReverseOn;
     On = true;
  }else if(digitalRead(20) == HIGH && digitalRead(14) == HIGH && digitalRead(9) == HIGH && digitalRead(10) == HIGH && On == true) On = false;
  ...
  for (int i = 0; i <= 1; i++) {
    ...
    if (encoderPos[i] != 0) {
      ...
      if(encoderPos[0] > 5 && offL == false || encoderPos[1] < -5 && offL == false && LightsReverseOn == false){
       encoderSpeed();
       offL = true; 
      }
      if(encoderPos[0] < -2 && offR == false || encoderPos[1] > 5 && offR == false && LightsReverseOn == false){
        encoderSpeed(); 
        offR = true;
      }            
